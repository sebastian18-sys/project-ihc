...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
        $html .= '<div><a class="suggest-element" data="'.utf8_encode($row['nombre_mascota'].' de  '.$row['nombre']).'" id="cliente'.$row['id_cliente'].'">'.utf8_encode($row['nombre_mascota'].' de  '.$row['nombre']).'</a></div>';
    }
}
echo $html;
?>